...
@section('content')
    ...
    <div class="table-container">
        ...
        {{-- Search Bar --}}
        <form method="GET" action="{{ route('products.index') }}" style="margin-bottom: 20px; display: flex; gap: 10px; align-items: center;">
            <div style="position: relative; flex: 1; max-width: 400px;">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama produk atau kategori..." style="width: 100%; padding: 9px 12px 9px 38px; border: 1px solid #ddd; border-radius: 8px; font-family: 'Nunito', sans-serif; font-size: 14px; outline: none; box-sizing: border-box;" onfocus="this.style.borderColor='var(--cokelat-utama)'" onblur="this.style.borderColor='#ddd'">
            </div>
            <button type="submit" class="btn-tambah" style="padding: 9px 20px;">Cari</button>
            @if($search)
                <a href="{{ route('products.index') }}" style="padding: 9px 16px; border-radius: 8px; border: 1px solid #ddd; background: #f9f9f9; color: #666; font-size: 14px; text-decoration: none; font-family: 'Nunito', sans-serif;">✕ Reset</a>
            @endif
        </form>
        @if($search)
            <p style="font-size: 13px; color: #888; margin-bottom: 12px;">Menampilkan hasil untuk: <strong>"{{ $search }}"</strong> ({{ $products->total() }} ditemukan)</p>
        @endif

...
@section('content')
    <div class="form-container" style="max-width: 800px; margin: 0 auto; background-color: var(--putih); padding: 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
        <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 25px;">
            ...
            <h2 style="margin: 0; font-family: 'Lora', serif; color: var(--cokelat-gelap);">Ubah Detail Transaksi #ORD-{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</h2>
        </div>
        ...
        <form action="{{ route('orders.update', $order->id) }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 20px;">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="quantity" id="quantity" class="form-control" value="{{ old('quantity', $quantity) }}" required min="1" style="width: 100%; padding: 12px 15px; border: 1px solid #eae0d5; border-radius: 8px; font-family: 'Nunito', sans-serif; font-size: 14px;">
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 20px;">
                <div class="form-group">
                    ...
                    <select name="size" id="size" class="form-control" required style="width: 100%; padding: 12px 15px; border: 1px solid #eae0d5; border-radius: 8px; font-family: 'Nunito', sans-serif; font-size: 14px;">
                        <option value="S" {{ old('size', $size) == 'S' ? 'selected' : '' }}>S (Small)</option>
                        <option value="M" {{ old('size', $size) == 'M' ? 'selected' : '' }}>M (Medium)</option>
                        <option value="L" {{ old('size', $size) == 'L' ? 'selected' : '' }}>L (Large)</option>
                        <option value="XL" {{ old('size', $size) == 'XL' ? 'selected' : '' }}>XL (Extra Large)</option>
                        <option value="XXL" {{ old('size', $size) == 'XXL' ? 'selected' : '' }}>XXL (Double Extra Large)</option>
                        <option value="Custom" {{ old('size', $size) == 'Custom' ? 'selected' : '' }}>Custom (Tulis di Catatan)</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control" required style="width: 100%; padding: 12px 15px; border: 1px solid #eae0d5; border-radius: 8px; font-family: 'Nunito', sans-serif; font-size: 14px;">
                        <option value="menunggu" {{ old('status', $order->status) == 'menunggu' ? 'selected' : '' }}>Menunggu Pembayaran</option>
                        <option value="proses" {{ old('status', $order->status) == 'proses' ? 'selected' : '' }}>Dalam Proses Jahit</option>
                        <option value="selesai" {{ old('status', $order->status) == 'selesai' ? 'selected' : '' }}>Selesai Dijahit</option>
                        <option value="diambil" {{ old('status', $order->status) == 'diambil' ? 'selected' : '' }}>Sudah Diambil</option>
                        <option value="batal" {{ old('status', $order->status) == 'batal' ? 'selected' : '' }}>Batal / Cancel</option>
                    </select>
                </div>
            </div>
            ...
            <div style="margin-top: 30px; display: flex; justify-content: flex-end; gap: 10px;">
                ...
                <button type="submit" class="btn-submit" style="padding: 12px 25px; border-radius: 8px; font-weight: bold; cursor: pointer; line-height: 1;">Perbarui Transaksi</button>
            </div>
        </form>
    </div>
@endsection

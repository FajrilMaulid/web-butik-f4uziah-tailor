...
- **Keranjang Belanja Premium**:
    ...
    - Mengatur jumlah kuantitas (_quantity_) pembelian.
...
- **Manajemen Profil Mandiri**:
    ...
    - Upload foto profil (_avatar_) dengan sistem manajemen penyimpanan file yang rapi.
...
- **Manajemen Pesanan / Orders (CRUD)**:
    ...
    - Memperbarui status pesanan (e.g., _menunggu_, _diproses_, _selesai_, _dibatalkan_).
...
Database Seeder telah mengonfigurasi dua akun dengan peran (_role_) berbeda untuk kebutuhan simulasi & pengujian:
...
- **Tata Letak Footer**: Deskripsi singkat, nomor telepon tambahan footer, username Instagram, dan teks hak cipta (_copyright_) dapat disesuaikan secara dinamis.
- **Tautan Sosial Media Aktif**: Username Instagram yang dimasukkan oleh admin akan otomatis tersemat di bagian footer sebagai link aktif (_clickable link_) ke profil Instagram butik dengan efek hover warna cokelat utama khas yang premium.
...
- _Dilengkapi dengan retensi pencarian berbasis query-string saat berpindah halaman (pagination)._
...
- **Hero Responsive CSS**: Menata ulang posisi hero banner di perangkat mobile. Gambar hero kini diposisikan secara seimbang di atas tombol _Call-to-Action (CTA)_, bukan disembunyikan seluruhnya ataupun ditaruh di bawah tombol yang dapat merusak alur visual pembeli.
...
### 5. 🛒 Penggabungan Transaksi Banyak Produk (Grouped Order Payments)

Masalah sebelumnya di mana pembelian beberapa produk sekaligus dari keranjang menghasilkan nomor order, total harga, dan bukti transfer terpisah telah diselesaikan secara menyeluruh:

- **Nomor Transaksi Bersatu (`payment_code`)**: Checkout banyak produk sekaligus dalam satu sesi keranjang akan menghasilkan satu kode transaksi gabungan (format `ORD-YYYYMMDD-[RANDOM]`) di database.
- **Pembayaran Terpadu**: Halaman informasi pembayaran menampilkan daftar seluruh produk, total nominal tagihan gabungan, serta pesan konfirmasi WhatsApp yang terintegrasi.
- **Upload Bukti Transfer Tunggal**: Pengguna hanya perlu mengunggah bukti transfer satu kali saja di halaman profil. Bukti tersebut akan otomatis diterapkan ke seluruh produk dalam grup transaksi tersebut.
- **Sinkronisasi Status Grup & Admin**:
    - Ketika admin menyetujui atau menolak bukti pembayaran salah satu pesanan di grup, status pembayaran seluruh pesanan dalam grup tersebut akan diperbarui secara otomatis.
    - Ketika admin mengubah status pesanan (misal ke _proses_, _selesai_, _diambil_, _batal_) melalui dropdown cepat atau form edit detail di Admin Panel, status semua produk dalam grup transaksi yang sama akan otomatis tersinkronisasi.
- **Tampilan Riwayat Rapi**: Pada halaman profil pengguna, produk yang dibeli bersamaan akan ditampilkan dalam satu kartu riwayat pemesanan terpadu dengan satu nomor order, status gabungan, total harga grup, dan tombol aksi tunggal.

### 6. 🔗 Pengalihan Halaman & Retensi Posisi Scroll yang Cerdas

- **Pengalihan Otomatis ke Tab Riwayat**: Setelah sukses melakukan checkout atau mengunggah bukti transfer, pengguna akan langsung dialihkan ke halaman profil dengan tab **History Pemesanan** yang langsung aktif (menggunakan parameter `?tab=history`), memangkas kebingungan navigasi.
- **Retensi Posisi Katalog**: Menambahkan produk ke keranjang belanja kini akan mengalihkan kembali pengguna ke posisi penelusuran (menggunakan hash `#search`), menjaga agar pengguna tidak terlempar ke bagian paling atas halaman utama (hero) dan dapat langsung melanjutkan pencarian atau filter kategori dengan mudah.
